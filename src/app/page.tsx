...
import { auth, db } from '@/lib/firebase'; // Import Firestore and Auth instance
...
export default function TenantBranchSelectionPage() {
  ...
  const [isLoadingTenants, setIsLoadingTenants] = useState<boolean>(false); // For tenant data fetching specifically
  const [isLoadingBranches, setIsLoadingBranches] = useState<boolean>(false); // For branch data fetching
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        setCurrentUser(null);
        // Clear data and selections if user signs out
        setTenantsData([]);
        setBranchesData([]);
  ...
  useEffect(() => {
    if (isAuthLoading) {
      ...
      setIsLoadingTenants(true); // Indicate loading while auth check is in progress
      return;
    }
    ...
    if (!currentUser) {
      ...
      setTenantsData([]);
      setIsLoadingTenants(false);
      // Error message is set by auth listener, or can be set here if needed
      if (!isAuthLoading) setFetchError("Please log in to select a tenant and branch.");
      return;
    }

    // Proceed with fetching tenants if currentUser exists
    ...
    const unsubscribeTenants = onSnapshot(tenantsQuery, (querySnapshot) => {
      ...
      setIsLoadingTenants(false);
      console.log("Tenant/Branch Page: Tenants data updated:", fetchedTenants);
      if (fetchedTenants.length === 0) {
        console.warn("Tenant/Branch Page: No tenants found in 'Tenants' collection.");
        setFetchError("No tenants found for your account, or you may not have permission to view them.");
      }
    }, (error) => {
      console.error("Tenant/Branch Page: Error fetching tenants with onSnapshot:", error);
  ...
  useEffect(() => {
    ...
    if (!selectedTenantId) {
      setBranchesData([]);
      setSelectedBranchId(undefined);
      setIsLoadingBranches(false); // Ensure loading is false if no tenant
      return;
    }

    setFetchError(null);
    setIsLoadingBranches(true);
    console.log(`Tenant/Branch Page: Authenticated user. Setting up Firestore listener for Branches of tenant: ${selectedTenantId}`);
    ...
    const unsubscribeBranches = onSnapshot(branchesQuery, (querySnapshot) => {
      ...
      setIsLoadingBranches(false);
      console.log("Tenant/Branch Page: Branches data updated:", fetchedBranches);
       if (fetchedBranches.length === 0) {
        console.warn(`Tenant/Branch Page: No branches found for tenantId '${selectedTenantId}' in 'Branches' collection.`);
        setFetchError(`No branches found for the selected tenant, or you may not have permission.`);
      }
    }, (error) => {
      console.error(`Tenant/Branch Page: Error fetching branches for tenant ${selectedTenantId} with onSnapshot:`, error);
  ...
  const getTenantPlaceholder = () => {
    ...
    if (!currentUser && !isAuthLoading) return "Please log in to select tenant";
  ...
  const getBranchPlaceholder = () => {
    ...
    if (isAuthLoading) return "Authenticating...";
    if (!currentUser && !isAuthLoading) return "Please log in to select branch";
    if (isLoadingBranches) return "Loading branches...";
    if (branchesData.length === 0 && !fetchError && currentUser) return "No branches for this tenant";
      ...
      <Card className="w-full max-w-md shadow-2xl z-10">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-muted-foreground">
            {isAuthLoading ? "Checking authentication status..." : currentUser ? "Please select your tenant and branch." : "Please log in to continue."}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {fetchError && (!isAuthLoading && (!currentUser || (currentUser && (tenantsData.length === 0 || (selectedTenantId && branchesData.length === 0 ))))) && (
            ...
            <>
              <div className="space-y-2">
                ...
                <Select 
                  ...
                  disabled={isAuthLoading || (!currentUser && !isAuthLoading) || isLoadingTenants || (tenantsData.length === 0 && !fetchError && !!currentUser)}
                >
                  <SelectTrigger id="tenant-select" className="w-full">
                    {(isAuthLoading || isLoadingTenants) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                    <SelectValue placeholder={getTenantPlaceholder()} />
                  </SelectTrigger>
                  <SelectContent>
                    {isAuthLoading ? (
                      <div className="flex items-center justify-center p-2"><Loader2 className="mr-2 h-4 w-4 animate-spin" /><span>Authenticating...</span></div>
                    ) : !currentUser && !isAuthLoading ? (
                    ...
                    ) : tenantsData.length === 0 && !isLoadingTenants && !fetchError && !!currentUser ? (
              ...
              {selectedTenantId && (currentUser || isAuthLoading) && (
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    disabled={isAuthLoading || (!currentUser && !isAuthLoading) || !selectedTenantId || isLoadingBranches || (branchesData.length === 0 && !fetchError && !!currentUser)}
                  >
                    ...
                    <SelectContent>
                      {isAuthLoading && selectedTenantId ? (
                         <div className="flex items-center justify-center p-2"><Loader2 className="mr-2 h-4 w-4 animate-spin" /><span>Authenticating...</span></div>
                      ) : !currentUser && !isAuthLoading && selectedTenantId ? (
                          <SelectItem value="login-required" disabled>Please log in</SelectItem>
                      ...
                      ): branchesData.length === 0 && !isLoadingBranches && selectedTenantId && !fetchError && !!currentUser ? (
